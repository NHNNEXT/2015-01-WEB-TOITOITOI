				...
				post_ul
						.insertAdjacentHTML(
								...
								'<li class="post" data-key='
										...
										+ '<div class="info">'
										+ '<span class="like-post" data-key='
										+ post.pid
										+ ' name="likesOnPost" action="/likedOnPost">'
										+ post.liked
										+ '</span>'
										+ '<span class="replies-post">'
										+ 0
										+ '</span>'
										+ '<div class="time">'
										+ post.creattime.split(" ")[0]
										+ '</div>'
										+ '</div>'
										+ '<ul class="replies"></ul>'
										+ '<div class="replyBox">'
			...
			.forEach(function(reply) {
				reply_ul
						.insertAdjacentHTML(
								'beforeend',
								'<li class="reply">'
										+ '<div class="reply-content">re: '
										+ reply.replyContent
										+ '</div>'
										+ '<div class="like-reply" value="'
										+ reply.reId
										+ '" name ="likesOnReply" action="/likedOnReply" method= "post" >'
										+ reply.liked + '</div>' + '</li>');
			});
...
function updateReplies(reply, replyList, replyCount) {
	replyList.insertAdjacentHTML('beforeend', "<li class='reply'>"
			+ "<div class='reply-content'> re: " + reply.replyContent
			+ "</div>" + "<div class='like-reply' value='" + reply.reId
			+ "' name ='likesOnReply' action='/likedOnReply' method= 'post' >"
			+ reply.liked + "</div>" + "</li>");
	...
	postList
			.insertAdjacentHTML(
					...
					'<li class="post" data-key='
							...
							+ '<input name="pid" type="hidden" value="'
							+ post.pid + '">'
							+ '<button class="send">게시</button>' + '</form>'
							+ '</div>' + '</li>');
}
...
function changeNudge() {
	var nextNudge = (nudges.length) ? nudges[(nudgeIndex++) % nudges.length]
			: '이 카페 아메리카노 어때요?';
...
$('.posting input').on(
		...
		function(e) {
			...
			var changedArray = toBeChangedElement
